...
# Main telemetry file
TELEMETRY_FILE = (
    "Drone onboard multi-modal sensor dataset for complex outdoor scenarios.csv"
)
...
OUTPUT_FILE = "drone_onboard_multimodal_cleaned.csv"
...
WIND_ANGLE_UNIT = "degrees"

# True keeps every original column after the standardized columns.
# False creates a smaller standardized output.
KEEP_ORIGINAL_COLUMNS = False

# Dataset-specific unit conversion: the source stores millivolts/milliamperes.
BATTERY_VOLTAGE_SCALE = 0.001
BATTERY_CURRENT_SCALE = 0.001
# Source position_x is latitude and position_y is longitude; the project
# convention for geographic data is X=longitude and Y=latitude.
SWAP_XY = True
COORDINATE_FRAME = "geographic_lon_lat_alt"
...
# ============================================================
# AMOVFLY-SPECIFIC COLUMN ALIASES
# ============================================================
...
for alias_group, new_names in AMOVFLY_ALIASES.items():
    for name in new_names:
        if name not in ALIASES[alias_group]:
            ALIASES[alias_group].append(name)
...
def detect_csv_separator(path):
    """Detect the delimiter from the header while retaining pandas' fast parser."""
    with Path(path).open("r", encoding="utf-8-sig", errors="replace") as handle:
        header = handle.readline()
    counts = {separator: header.count(separator) for separator in [",", ";", "\t", "|"]}
...
def standardize_drone_data(
    ...
    battery_current_scale=1.0,
    swap_xy=False,
    coordinate_frame_override=None,
):
    """
    ...
    raw_telemetry = pd.read_csv(
        telemetry_path,
        sep=detect_csv_separator(telemetry_path),
        low_memory=False,
    )
    ...
    if metadata_file is not None:
        ...
        raw_metadata = pd.read_csv(
            metadata_path,
            sep=detect_csv_separator(metadata_path),
            low_memory=False,
        )
    ...
    if swap_xy:
        ...
        mapping["X"] = f"{y_column} (longitude; swapped from source Y)"
        mapping["Y"] = f"{x_column} (latitude; swapped from source X)"
    ...
    battery_voltage = battery_voltage * float(battery_voltage_scale)
    battery_current = battery_current * float(battery_current_scale)
    ...
    if coordinate_frame_override:
        coordinate_frame = coordinate_frame_override
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Standardize drone telemetry for the wind-model pipeline."
    )
    parser.add_argument("--input", default=TELEMETRY_FILE)
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=KEEP_ORIGINAL_COLUMNS,
    )
    parser.add_argument(
        "--battery-voltage-scale", type=float, default=BATTERY_VOLTAGE_SCALE
    )
    parser.add_argument(
        "--battery-current-scale", type=float, default=BATTERY_CURRENT_SCALE
    )
    parser.add_argument("--no-swap-xy", action="store_false", dest="swap_xy")
    parser.set_defaults(swap_xy=SWAP_XY)
    parser.add_argument("--coordinate-frame", default=COORDINATE_FRAME)
    cli_args = parser.parse_args()

    try:
        cleaned_data = standardize_drone_data(
            telemetry_file=cli_args.input,
            metadata_file=cli_args.metadata,
            output_file=cli_args.output,
            source_dataset=cli_args.source,
            keep_original_columns=cli_args.keep_original_columns,
            battery_voltage_scale=cli_args.battery_voltage_scale,
            battery_current_scale=cli_args.battery_current_scale,
            swap_xy=cli_args.swap_xy,
            coordinate_frame_override=cli_args.coordinate_frame,
        )
