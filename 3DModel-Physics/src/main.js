...
function updatePhysicsEstimate(deltaTime) {
  if (physicsModelInput.value === "identified") {
    const estimate = inferIdentifiedWind(telemetry);
    const targetWind = new THREE.Vector3(estimate.x, estimate.y, estimate.z);
    smoothedWind.lerp(targetWind, 1 - Math.exp(-5 * deltaTime));
    ...
    telemetry.aerodynamicForceX = 0;
    telemetry.aerodynamicForceY = 0;
    telemetry.aerodynamicForceZ = 0;
    telemetry.aerodynamicForceMagnitude = 0;
